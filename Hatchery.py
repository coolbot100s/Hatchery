...
def make_many_fish(input_dir, output_dir, modinfo):
    ...
    with open(input_dir, mode="r") as csvfile:
        ...
        for row in reader:
            ...
            if "mod_loot_table" in data:
                mod_tables.append(data["mod_loot_table"])

    return fish_list, mod_tables
...
# Code stuff
def create_main_gd(modinfo, fish_list, colors_list, titles_list, mod_tables):
    content = f"""
...
"""
    ...
    for i, fish in enumerate(fish_list):
        mod_table = ""
        if mod_tables and i < len(mod_tables) and mod_tables[i]:
            table_list = mod_tables[i].split(',')
            loot_tables = [f"Lure.loot_table('{table.strip()}')" for table in table_list]
            mod_table = f""", [{', '.join(loot_tables)}]"""
        content += f"""    Lure.add_content(ID,"{fish}","mod://scenes/fish/{fish}.tres"{mod_table}) \n"""
...
def new_mod():
    ...
    if add_fish_to_mod_choice == "Yes":
        fish_list, fish_data_dir, mod_tables = new_fish(True, modinfo)
        adds_fish = True
    ...
    if create_main_class_choice == "Yes":
        create_main_gd(modinfo, fish_list, colors_list, titles_list, mod_tables)
...
def new_fish(making_mod, modinfo): 
    ...
    if csv_or_cli_choice == "I have a .csv file":
        ...
        fish_list, mod_tables = make_many_fish(input_dir, output_dir, modinfo)
    else:
        print("This feature is not yet supported, Sorry!")
        new_fish(making_mod, modinfo)
    
    return fish_list, input_dir, mod_tables
